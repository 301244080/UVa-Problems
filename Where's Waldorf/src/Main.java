...
public class Main {
    ...
    private static boolean dfs(SuffixArray[] suffixArrays, int rowPos, int rowSize, int colPos, int colSize, char[] wordArr, int wordPos, int pos){
    	...
    	if(wordPos < wordArr.length){
    		if(rowPos-1 >= 0&&pos==2)
    		{
    			...
    			for(int topCheckNum:topCheckNums){               	
                    ...
                    if(topCheckNum == colPos-1){
                    	
                        if(dfs(suffixArrays, rowPos-1, rowSize, topCheckNum, colSize, wordArr,wordPos+1,pos)) 
                        	{
    		...
    		if(rowPos+1 < rowSize&&pos==8)
    		{
    			bottomCheckNums = binarySearch(suffixArrays[rowPos+1], colSize, wordArr[wordPos]);
    			for(int bottomCheckNum: bottomCheckNums){
                	
                    if(bottomCheckNum == colPos-1){
                        if(dfs(suffixArrays, rowPos+1, rowSize, bottomCheckNum, colSize, wordArr, wordPos+1,pos)) return true;
                    }
                }
    		}
    		if(rowPos+1 < rowSize&&pos==7)
    		{
    			bottomCheckNums = binarySearch(suffixArrays[rowPos+1], colSize, wordArr[wordPos]);
    			for(int bottomCheckNum: bottomCheckNums){
                	
                    if(bottomCheckNum == colPos){
                        if(dfs(suffixArrays, rowPos+1, rowSize, bottomCheckNum, colSize, wordArr, wordPos+1,pos)) return true;
                    }
                }
    		}
    		if(rowPos+1 < rowSize&&pos==6)
    		{
    			bottomCheckNums = binarySearch(suffixArrays[rowPos+1], colSize, wordArr[wordPos]);
    			for(int bottomCheckNum: bottomCheckNums){
    		...
    		if(colPos-1 >= 0&&pos==1)
    		{
                ...
                for(int leftCheckNum:leftCheckNums){
                    ...
                    if(leftCheckNum == colPos-1){
                        ...
                        if(dfs(suffixArrays,rowPos, rowSize, leftCheckNum,colSize,wordArr, wordPos+1,pos)) return true;
                    }
                }
                
            }
    		if(colPos+1 < colSize&&pos==5)
    		{
                ...
                for(int rightCheckNum:rightCheckNums){
                    if(rightCheckNum == colPos +1){
                        if(dfs(suffixArrays,rowPos,rowSize,rightCheckNum,colSize,wordArr,wordPos+1,pos)) return true;
                    }
                }
            }
    	}
    ...
    private static boolean bfs(SuffixArray[] suffixArrays, int rowPos, int rowSize, int colPos, int colSize, char[] wordArr, int wordPos){
       ...
       // System.out.println(rowPos + " col: " + colPos + " char: " + wordArr[wordPos-1]);
        if(wordPos < wordArr.length){
            ...
            if(rowPos-1 >= 0){
                topCheckNums = binarySearch(suffixArrays[rowPos-1],colSize,wordArr[wordPos]);
                //System.out.println("aa "+topCheckNums);
                for(int topCheckNum:topCheckNums){
                	if(topCheckNum+1==colPos)
                	{
                		pos=2;
                	}
                	if(topCheckNum==colPos)
        ...
        while(rowPos< rowSize && rowPos >=0 && wordPos >=0 && wordPos<wordArr.length){
            ...
            if( currRes.get(0)!= -2){
                for(int i=0; i<currRes.size();i++){
                    ...
                    if(bfs(suffixArrays, rowPos, rowSize, colPos, colSize, wordArr, wordPos+1)) 
                    	{
